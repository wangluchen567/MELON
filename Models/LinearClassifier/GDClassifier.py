...
class GDClassifier(Model):
    ...
    def cal_loss_(self):
        ...
        if self.loss == "hinge":
            ...
            # 计算损失值（根据间隔的平方损失）
            self.Loss = np.sum(np.maximum(0, 1 - self.Y_train * self.X_dot_W) ** 2) / len(self.X_train)
        else:
    ...
    def cal_grad_(self):
        ...
        if self.loss == "hinge":
            # 得到分类错误的掩码向量（根据间隔）
            error_mask = ((self.Y_train * self.X_dot_W) < 1).astype(float)
            # 求梯度向量
            ...
            # 得到分类错误的掩码向量（根据间隔的平方损失）
            error_mask = ((self.Y_train * self.X_dot_W) < 1).astype(float)
            # 求梯度向量
            self.Grad = - np.sum(2 * self.Y_train * self.X_train_B * (1 - self.Y_train * self.X_dot_W) * error_mask,
                                 axis=0).reshape(-1, 1) / len(self.X_train)
        else:
            raise ValueError(f"Unsupported loss: {self.loss}")
