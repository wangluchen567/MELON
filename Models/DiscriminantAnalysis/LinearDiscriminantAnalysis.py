...
class LinearDiscriminantAnalysis():
    ...
    def set_parameters(self, priors=None, n_components=None):
        """重新修改相关参数"""
        parameters = ['priors', 'n_components']
        values = [priors, n_components]
        for param, value in zip(parameters, values):
            if value is not None and getattr(self, param) is not None:
                warnings.warn(f"Parameter '{param}' will be overwritten")
                setattr(self, param, value)

    def train(self, X_train=None, Y_train=None, priors=None, n_components=None):
        """使用数据集训练模型"""
        self.set_train_data(X_train, Y_train)
        self.set_parameters(priors, n_components)
